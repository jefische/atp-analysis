...
train$P1Wins <- factor(train$P1Wins)
test$P1Wins <- factor(test$P1Wins)
...
m1.gbm.caret <- train(P1Wins ~ P1_Elo + P2_Elo + P1_Rank + P2_Rank + P1_P_Win + P2_P_Win + P1_P_Sets + P2_P_Sets, 
                      data=train, method = "gbm", distribution = "bernoulli",
                      verbose = FALSE)

m1.gbm.caret
summary(m1.gbm.caret)
p.boost <- predict(m1.gbm.caret, newdata = test)
p.boost

table(Predicted=p.boost, Actual=test$P1Wins)
sum(diag(table(p.boost, test$P1Wins)))/sum(table(p.boost, test$P1Wins)) #80%


myGrid <- expand.grid(n.trees = c(150, 300, 500),
                      interaction.depth = c(1, 2, 3, 4, 5, 6),
                      shrinkage = c(0.01, 0.10, 0.15),
                      n.minobsinnode = c(10))

m2.gbm.caret <- train(P1Wins ~ P1_Elo + P2_Elo + P1_Rank + P2_Rank + P1_P_Win + P2_P_Win + P1_P_Sets + P2_P_Sets, 
                      data=train, method = "gbm", distribution = "bernoulli",
                      verbose = FALSE, tuneGrid = myGrid)
